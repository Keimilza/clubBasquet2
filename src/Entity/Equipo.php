...
class Equipo
{
    ...
    public function addResultadolocal(Resultado $resultadolocal): self
    {
        if (!$this->resultadoslocal->contains($resultadolocal)) {
            $this->resultadoslocal[] = $resultadolocal;
            $resultadolocal->setEquipolocal($this);
        }

        return $this;
    }

    public function removeResultadolocal(Resultado $resultadolocal): self
    {
        if ($this->resultadoslocal->contains($resultadolocal)) {
            $this->resultadoslocal->removeElement($resultadolocal);
            // set the owning side to null (unless already changed)
            if ($resultadolocal->getEquipolocal() === $this) {
                $resultadolocal->setEquipolocal(null);
            }
        }
